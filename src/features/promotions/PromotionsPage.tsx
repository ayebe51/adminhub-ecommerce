...
interface RuleBuilderModalProps {
  ...
  initialData?: Partial<PromotionRule> | null;
}

const RuleBuilderModal: React.FC<RuleBuilderModalProps> = ({ isOpen, onClose, onSave, editingRule, initialData }) => {
  const [name, setName] = useState(editingRule?.name || initialData?.name || '');
  const [description, setDescription] = useState(editingRule?.description || initialData?.description || '');
  const [conditions, setConditions] = useState<Condition[]>(
    editingRule?.conditions?.conditions?.filter((c): c is Condition => 'type' in c) || 
    initialData?.conditions?.conditions?.filter((c): c is Condition => 'type' in c) || []
  );
  const [conditionOp, setConditionOp] = useState<'AND' | 'OR'>(
    editingRule?.conditions?.operator || initialData?.conditions?.operator || 'AND'
  );
  const [actions, setActions] = useState<PromotionAction[]>(editingRule?.actions || initialData?.actions || []);
  const [startDate, setStartDate] = useState(editingRule?.schedule?.startDate?.slice(0, 10) || initialData?.schedule?.startDate?.slice(0, 10) || '');
  const [endDate, setEndDate] = useState(editingRule?.schedule?.endDate?.slice(0, 10) || initialData?.schedule?.endDate?.slice(0, 10) || '');
...
export const PromotionsPage: React.FC = () => {
  ...
  const [initialData, setInitialData] = useState<Partial<PromotionRule> | null>(null);
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [aiInput, setAiInput] = useState('');
  const [isAiProcessing, setIsAiProcessing] = useState(false);

  const parseNaturalLanguage = (text: string): Partial<PromotionRule> => {
    const t = text.toLowerCase();
    const rule: Partial<PromotionRule> = {
      name: text,
      description: 'Generated by AI Magic Box',
      conditions: { operator: 'AND', conditions: [] },
      actions: [],
      schedule: { startDate: new Date().toISOString(), endDate: null, timezone: 'UTC', isRecurring: false, recurringDays: [], recurringStartTime: null, recurringEndTime: null },
    };

    // Parse Actions
    if (t.includes('%') || t.includes('percent')) {
      const match = t.match(/(\d+)\s*(%)|(percent)/);
      if (match) {
        rule.actions?.push({
          id: uuid(), type: 'percentage_discount', value: parseInt(match[1]),
          target: 'order', targetProductIds: [], targetCategoryIds: [], maxDiscount: null
        });
      }
    } else if (t.includes('$') || t.includes('off')) {
      const match = t.match(/\$(\d+)|(\d+)\s*off/);
      if (match) {
        rule.actions?.push({
          id: uuid(), type: 'fixed_discount', value: parseInt(match[1] || match[2]),
          target: 'order', targetProductIds: [], targetCategoryIds: [], maxDiscount: null
        });
      }
    } else if (t.includes('free shipping')) {
      rule.actions?.push({ id: uuid(), type: 'free_shipping', value: 0, target: 'shipping', targetProductIds: [], targetCategoryIds: [], maxDiscount: null });
    }

    // Parse Conditions
    if (t.includes('over') || t.includes('above') || t.includes('more than')) {
      const match = t.match(/(?:over|above|more than)\s*(?:\$)?\s*(\d+)/);
      if (match) {
        (rule.conditions?.conditions as Condition[]).push({
          id: uuid(), type: 'cart_total', field: 'subtotal', operator: 'greater_than', value: parseInt(match[1])
        });
      }
    }
    ...
    for (const cat of categories) {
      if (t.includes(cat.toLowerCase())) {
        (rule.conditions?.conditions as Condition[]).push({
          id: uuid(), type: 'product_category', field: 'category', operator: 'equals', value: cat
        });
      }
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <div style={{ 
            ...
            background: 'linear-gradient(135deg, var(--color-brand-500), var(--color-text-danger))',
        ...
        <form onSubmit={handleAiSubmit} style={{ display: 'flex', gap: '12px' }}>
          <div style={{ flex: 1, position: 'relative' }}>
            <input
              type="text"
              placeholder='e.g., "Give 20% discount on Electronics for orders over 500"'
              value={aiInput}
            ...
            <div style={{ 
              ...
              fontSize: '11px', color: 'var(--color-text-tertiary)', pointerEvents: 'none'
            }}>
              Press Enter ↵
            </div>
          </div>
          <button
            ...
            style={{
              ...
              borderRadius: 'var(--radius-xl)',
              border: 'none',
              background: 'linear-gradient(135deg, var(--color-brand-600), var(--color-text-danger))',
              color: 'white',
              fontSize: '14px',
              fontWeight: 600,
              cursor: 'pointer',
              opacity: (isAiProcessing || !aiInput.trim()) ? 0.6 : 1,
        ...
        <div style={{ display: 'flex', gap: '20px', fontSize: '11px', color: 'var(--color-text-tertiary)' }}>
          <span>Try: "Free shipping on orders over $100"</span>
          <span>•</span>
          <span>"15% off for Electronics"</span>
          <span>•</span>
      ...
      <RuleBuilderModal
        key={showBuilder ? `open-${editingRule?.id || initialData?.name || 'new'}` : 'closed'}
        isOpen={showBuilder}
        onClose={() => { setShowBuilder(false); setEditingRule(null); setInitialData(null); }}
        onSave={handleSave}
